...
object GatewayClient {
    fun sendNotification(
        ...
        return runCatching {
            ...
            DebugLog.add(context, "POST $endpoint package=$packageName title=$title")
            ...
            if (status !in 200..299) {
                DebugLog.add(context, "POST failed HTTP $status: $response")
                error("HTTP $status: $response")
            }

            DebugLog.add(context, "POST success HTTP $status: $response")
            response
        }
    }
}
